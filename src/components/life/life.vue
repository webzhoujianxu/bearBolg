...
import dialogMent from '../dialog/dialog.vue'
@Component({
    components: {
        dialogMent
    }
})
export default class lifeIndex  extends Mixins(MixinTable) {
    private List ?: object
    ...
    private created () {
        ...
        * */
        this.eventConfig = {
            ...
            clickType: 0,
            EditIndex: 0,
            addUrl: 'lifes/uoloadLife',
            editUrl: 'lifes/editLife',
            ArrayKey: 'List',
            deleteUrl: 'lifes/deleteLife',
            name: '日志'
        }
    }
    private async getList (pageNum:number, stutes: number|string) {
      let result = await this.$axios.post('/lifes/findAllLife', {
            pageNum: pageNum,
            stutes: stutes
        })
    ...
    private async searchList (pageNum:number , stutes:number|string) {
        let result = await this.$axios.post('/lifes/searchBystutes', {
            pageNum: pageNum,
            stutes: stutes
        })
        this.List = result.data.list
    ...
    public operation (item:any) {
        let _this:any = this
        let url = item.stutes === 0 ? 'lifes/shLife' : item.stutes === 1 ? 'lifes/unshLife' : item.stutes === 3?'lifes/xjLife': ''
